...
# During-After_without_smoking_PRS_stratification_model
...
cox_model_during <- coxph(Surv(tstart, tstop, bc_diagnosis) ~ fin_status + hrt.status + meno_status + hyster_status + bmi + tdi + AgeMenarch + yob, data = t_data_during)
...
cox_model_after <- coxph(Surv(tstart, tstop, bc_diagnosis) ~ fin_status + hrt.status + meno_status + hyster_status + bmi + tdi + AgeMenarch + yob, data = t_data_after)
