...
fit <- coxph(Surv(tstart, tstop, bc_diagnosis) ~ oc_use + SmokingStatus + hrt.status + meno_status + hyster_status + bmi + tdi + AgeMenarch + yob + number_children, data = t_data_combined)
...
fit_2 <- coxph(Surv(tstart, tstop, bc_diagnosis) ~ oc_use + prs + SmokingStatus + hrt.status + meno_status + hyster_status + bmi + tdi + AgeMenarch + yob + number_children, data = t_data_combined)
...
fit_3 <- coxph(Surv(tstart, tstop, bc_diagnosis) ~ oc_use * prs + SmokingStatus + hrt.status + meno_status + hyster_status + bmi + tdi + AgeMenarch + yob + number_children, data = t_data_combined)
